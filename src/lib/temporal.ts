...
const GROWTH_RATES = {
    revenue: [0.60, 0.40, 0.30, 0.25, 0.20],
    units: [0.53, 0.35, 0.25, 0.20, 0.15],
    employees: [0.22, 0.18, 0.15, 0.12, 0.10],
    waitlist: [0.21, 0.15, 0.12, 0.10, 0.08],
...
export function financialsTableData(): {
    ...
    const endFY = latestFiscalYear()
    const data: { year: string; revenue: string; margin: string; units: string }[] = []
    for (let fy = 2020; fy <= endFY; fy++) {
        const m = fyMetrics(fy)
        data.push({
